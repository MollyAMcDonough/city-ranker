...
function City({ categories, setCategories, city, cityKeys }) {
    ...
    const cityData = cityKeys.map((k)=><td className="p-4 px-6 text-xs align-middle border-t-0 border-l-0 border-r-0 whitespace-nowrap" key={city[k].toString()}>{city[k].toString()}</td>)
    return (
        <tr>
